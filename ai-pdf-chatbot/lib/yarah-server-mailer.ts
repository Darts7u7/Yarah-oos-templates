...
// Better Auth's email callbacks fire server-side without an end-user
// session, so mint a short-lived "service" HS256 JWT (same secret as
// the bridge) and attach it as the bearer for client.emails.send().
