...
A developer-friendly CRM starter built with Next.js, shadcn/ui, and Yarah.
...
An authenticated CRM starter built with Next.js, shadcn/ui, and Yarah. It ships with a visual sales pipeline, lead activity tracking, client conversion flows, project management, and enough structure for developers to adapt it into their own CRM without rebuilding auth, database access, or storage from scratch.
...
Build a CRM with Yarah auth, database, storage, and RLS without starting from a blank dashboard.
...
- [Yarah](https://yarah.dev) authentication with email/password and OAuth providers
...
Demo: [democrm.yarah.dev](https://democrm.yarah.dev)
...
6. Run `npx @yarahdev/cli db import migrations/db_init.sql` against the linked Yarah project if you have not already applied the schema
...
2. Re-run the migration against your Yarah project
